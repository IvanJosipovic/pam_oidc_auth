...
public class Tests : IClassFixture<TestFixture>
{
    ...
    public async Task Valid()
    {
        ...
        (await pam_oidc_auth.PamModule.ValidateJwt(token, "some-app", "someuser@company.com", "sub", "http://oidc-server-mock:8080/.well-known/openid-configuration")).ShouldBeTrue();
    }

    [Fact]
    public async Task BadToken()
    {
        (await pam_oidc_auth.PamModule.ValidateJwt("fake", "some-app", "someuser@company.com", "sub", "http://oidc-server-mock:8080/.well-known/openid-configuration")).ShouldBeFalse();
    }
    ...
    {
        var token = await GetToken();
        (await pam_oidc_auth.PamModule.ValidateJwt(token, "some-app", "someuser2@company.com", "sub", "http://oidc-server-mock:8080/.well-known/openid-configuration")).ShouldBeFalse();
    }
    ...
    {
        var token = await GetToken();
        (await pam_oidc_auth.PamModule.ValidateJwt(token, "some-app2", "someuser@company.com", "sub", "http://oidc-server-mock:8080/.well-known/openid-configuration")).ShouldBeFalse();
    }
